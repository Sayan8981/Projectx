...
class ProjectXSeries:
    ...
    def reverse_api_extract(self, link_details,source):
        ...
        for kk in reverse_api_resp:
            ...
            if kk.get("data_source")==source and kk.get("type")=='Program' and kk.get("sub_type")=='SE':
                if kk.get("projectx_id") not in self.px_id:      
                    self.px_id.append(kk.get("projectx_id"))
            if (kk.get("data_source")=='GuideBox' or kk.get("data_source")=='Hulu') and kk.get("type")=='Program' and kk.get("sub_type")=='SE':
                if kk.get("projectx_id") not in self.px_id:      
                    self.px_id.append(kk.get("projectx_id"))        
    ...
    def duplicate_checking_series(self, series_data, source, writer,writer1,object_): 
        #import pdb;pdb.set_trace()     
        print("\nFunction duplicate_checking_series_called")
        self.id=eval(series_data.get("launch_id"))
        self.show_id=eval(series_data.get("series_id").encode())
        ...
        if self.series_title is not None:
            ...
            print ("projectx_ids : {},vudu_se_id: {}, vudu_SM_id: {}".format(self.px_id,self.id,self.show_id))
            if len(self.px_id) > 1:
