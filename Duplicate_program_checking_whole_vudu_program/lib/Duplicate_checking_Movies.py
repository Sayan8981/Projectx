...
class ProjectXMovies:
    ...
    def expiry_check(self, link_details):
        ...
        expired_api=initialization_file.preprod_version_expire_domain%(link_details,'Vudu')
    ...
    def reverse_api_extract(self, link_details,source):
        ...
        for kk in reverse_api_resp:
            ...
            if kk.get("data_source")==source and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))
            if (kk.get("data_source")=='GuideBox' or kk.get("data_source")=='Hulu') and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))        
    ...
    def search_api_response_validation(self, data_resp_search, source, px_id, link_expired, writer, duplicate,writer1,object_):
        ...
        if duplicate=='False' or duplicate=='':
            ...
            if search_px_id1:
                ...
                duplicate_api=initialization_file.duplicate_api_domain%(self.id,source,'MO')
    ...
    def duplicate_checking_movies(self, movie_data, source, writer,writer1,object_): 
        #import pdb;pdb.set_trace()     
        print("\nFunction duplicate_checking_movies_called")
        self.id=eval(movie_data.get("launch_id"))
        ...
        print ("projectx_ids : {}, vudu_mo_id: {}".format(self.px_id,self.id))
        if len(self.px_id) > 1:               
