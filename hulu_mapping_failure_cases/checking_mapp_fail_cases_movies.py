...
class movies_mapp_fail_cases:
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        self.default_param()
        inputFile="input_movies_mappFail/hulu_movies_mapp_fail_input"
        input_data=lib_common_modules().read_csv(inputFile)

        result_sheet='/output_automated_fail_cases_movies/result_%s_%s.csv'%(thread_name,datetime.date.today())
    ...
    def thread_pool(self):
        t1=Process(target=self.main,args=(1,"thread-1",59))
        t1.start()
