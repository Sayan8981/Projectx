...
#TODO: main operation
def main():
    inputFile="/input/TVShows_mapped_ROVI_to_GB1"
    result_sheet='/result/Result_mapping_SM_new_API.csv'
    ...
    with output_file as mycsvfile:
        ...
        for data in range(1,len(input_data)):
            ...
            gb_sm_id=eval(input_data[data][1])
            rovi_sm_id=eval(input_data[data][0])
            print("\n")
            print ({"total": initialization_file.total,"rovi_sm_id":rovi_sm_id,"gb_sm_id":gb_sm_id})
            try:
                px_ids=getting_px_ids(gb_sm_id,rovi_sm_id,initialization_file.source)
                checking_mapping_series.checking_mapping_series(gb_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                   px_ids["rovi_projectx_id"],writer,initialization_file.source)
            except (Exception,httplib.BadStatusLine,urllib2.HTTPError,socket.error,urllib2.URLError) as e:
                retry_count+=1
                print ("Exception caught ......................",type(e),gb_sm_id,rovi_sm_id)
                if retry_count<=5:
                   px_ids= getting_px_ids(gb_sm_id,rovi_sm_id,initialization_file.source)
                   checking_mapping_series.checking_mapping_series(gb_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                      px_ids["rovi_projectx_id"],writer,initialization_file.source)
