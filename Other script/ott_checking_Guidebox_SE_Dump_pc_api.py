...
def ott(start,name,end,id):
    ...
    conn1=pymysql.connect(user="projectx",passwd="projectx",host="18.232.135.33",db="projectx",port=3370)                                                 #"52.91.200.186"
    cur1=conn1.cursor()
    ...
    with w as mycsvfile:
	    ...
	    try:
                res1=mytable.aggregate([{"$skip":aa},{"$limit":1000},{"$match":{"$and":[{"show_type":"SE"},{"$or":[{"purchase_web_sources": {"$exists": True, "$ne": []}},{"subscription_web_sources": {"$exists": True, "$ne": []}},{"tv_everywhere_web_sources": {"$exists": True, "$ne": []}}]}]}},{"$project":{"gb_id":1,"show_id":1,"_id":0}}])
	        print "start_id: "+str(start)
                for i in res1:
	        print("thread_name:", name, "total count of GB ID: ", total ,"Not ingested Gb_id :", w, "Ott_link pass: ",t, "Duplicates/Additinal ott_contents :",h, "missing ott_contentsand additional/duplicates ott_contents :", v, "missing ott_contents: ", m, "Total fail : ",h+v+m, "Total pass: ", t)
	        ...
	        print "end_id: "+str(end)
                ...
                print gb_id 
                mapping_url="http://34.231.212.186:81/projectx/GuideBox/%d-SE/source"%gb_id   
                                ...
                                try:
                    	            ...
                    	            try:
                         	        ...
                         	        try:
                    			        ...
                    			        try:
                    			            a3=re.findall("\w+.*?\-(\w+.*?\w+\w+.\w.-*?\d*\w?)", b)[-1][0]
                    			            arr7.insert(j,a3)
                    			        except IndexError:
                                                    try:
	  		                                a3=re.findall("\w+.*?\-(\w+.*?\w+.-*\w.*?\w+)", b)[-1:][0]  #### 
                                                        arr7.insert(j,a3)
                                                    except IndexError:
    ...
    except (AttributeError, pymongo.errors.OperationFailure,_mysql_exceptions.OperationalError):
        print ("caught exception .............................................................................>")
        continue

    print("thread_name:", self.name, "total count of GB ID: ", total ,"Not ingested Gb_id :", w, "Ott_link pass: ",t, "Duplicates/Additinal ott_contents :",h, "missing ott_contentsand additional/duplicates ott_contents :", v, "missing ott_contents: ", m, "Total fail : ",h+v+m, "Total pass: ", t)
    print("gb_ids not ingested",GB_id, len(GB_id))
