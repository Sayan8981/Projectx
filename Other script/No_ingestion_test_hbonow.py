...
def ingestion():
    ...
    result_sheet='/No_ingestion_test_hbonow.csv'
    ...
    with w as mycsvfile:
        ...
        total2=0
	total3=0
	query1="select launch_id,show_type from hbonow_programs where expired=0"
	cur1.execute(query1)
	res1=cur1.fetchall()
        j=0
        k=0
	...
	u=0
	    ...
	    hbogo_projectx_id=[]
		    ...
		    print total,l,j,k
                    total=total+1
		    try:
		        url_hbogo="http://34.231.212.186:81/projectx/mappingfromsource?sourceIds=%s&sourceName=HBONOW&showType=MO" %i[0]
                    ...
                    for jj in data_resp_hbogo:
                        if jj["data_source"]=="HBONOW" and jj["type"]=="Program" and jj["sub_type"]=="MO":
			...
			print("total hbogo id MO:", total ,"No ingestion: ", l, "Multiple mapped content :", j, "Total Fail: ", l+j, "Pass: ", k)
                if i[1]=='SE':
                    total1=total1+1
		    try:
                        url_hbogo="http://34.231.212.186:81/projectx/mappingfromsource?sourceIds=%s&sourceName=HBONOW&showType=SE" %i[0]
                        response_hbogo=urllib2.Request(url_hbogo)
                        response_hbogo.add_header('Authorization','Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3')
                        resp_hbogo=urllib2.urlopen(response_hbogo)
                        data_hbogo=resp_hbogo.read()
                    ...
                    for jj in data_resp_hbogo:
                        if jj["data_source"]=="HBONOW" and jj["type"]=="Program" and jj["sub_type"]=="SE":
                            hbogo_projectx_id.append(jj["projectx_id"])
                    if len(hbogo_projectx_id)>1:
                        q=q+1
    		...
    		if i[1]=='SM':
                    total2=total2+1
		    try:
                        url_hbogo="http://34.231.212.186:81/projectx/mappingfromsource?sourceIds=%s&sourceName=HBONOW&showType=SM" %i[0]
                        response_hbogo=urllib2.Request(url_hbogo)
                        response_hbogo.add_header('Authorization','Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3')
                        resp_hbogo=urllib2.urlopen(response_hbogo)
                        data_hbogo=resp_hbogo.read()
                    ...
                    for jj in data_resp_hbogo:
                        if jj["data_source"]=="HBONOW" and jj["type"]=="Program" and jj["sub_type"]=="SM":
                            hbogo_projectx_id.append(jj["projectx_id"])
                    if len(hbogo_projectx_id)>1:
                        n=n+1
    ...
    print("total hbonow id :", total+total1+total2+total3 ,"total No ingestion: ", m+p+l+u, "Multiple mapped content :", s+q+n+j, "Total Fail: ", m+p+l+q+n+j+s+u, "Pass: ", r+o+k+t)
