...
def id_search_amazon(start,name,end,id):
    ...
    result_sheet='/reverse_api_checked_amazon_guidebox_same_source_id_API%d.csv'%id
    if(os.path.isfile(os.getcwd()+result_sheet)):
        os.remove(os.getcwd()+result_sheet)
    csv.register_dialect('excel',lineterminator = '\n',skipinitialspace=True,escapechar='')
    w=open(os.getcwd()+result_sheet,"wa")
    with w as mycsvfile:
	fieldnames = ["gb_id","show_type","title","description","original_title","series_title","alternate_titles","season_number","episode_number","release_year","air_date","duration","content_type","updated_at_db","purchase_flag","subscription_flag","tv_everywhere_flag","free_web_flag","amazon_purchase_flag","amazon_purchase_id","amazon_subscription_flag","amazon_subscription_id","amazon_tv_everywhere_flag","amazon_tv_everywhere_id","amazon_free_web_flag","amazon_free_web_id","Rovi_id","multiple_id_flag_amazon","Amazon_link_projectx_id","comment"]
	...
	for mm in range(start,end,1000):
		...
		amazon_free_web_id_arr=[]
		amazon_purchase_id_arr=[]
		amazon_subscription_id_arr=[]
		amazon_tv_everywhere_id_arr=[]
		arr_px_amazon=[]
		arr_gb_amazon=[]
		arr_rovi_amazon=[]
		sec_arr_amazon=[]
		multiple_id_flag_amazon='False'
		amazon_purchase_flag=''
		amazon_subscription_flag=''
		amazon_tv_everywhere_flag=''
		amazon_free_web_flag=''
		comment1=''
		...
		if ii.get("purchase_web_sources")!=[] and ii.get("purchase_web_sources") is not None:
		    ...
		    purchase=ii.get("purchase_web_sources")  
		    for aa in purchase:
		        if "amazon_buy" in aa.get('source') or "amazon_prime" in aa.get('source'):
		            amazon_purchase_flag='True'
		            amazon_link1=(aa.get("link")).encode("utf-8")
		            amazon_purchase_id=re.findall("\w+\d+\w+", amazon_link1)[0]
		            amazon_purchase_id_arr.append(amazon_purchase_id)
		            break
		        else:         
		            amazon_purchase_flag='False'
		else:
		    purchase_flag='False'
		    amazon_purchase_flag='False'
		if ii.get("subscription_web_sources")!=[] and ii.get("subscription_web_sources") is not None:
		    ...
		    for bb in subscription:
	                if "amazon_buy" in bb.get('source') or "amazon_prime" in bb.get('source'):
		            amazon_subscription_flag='True'
		            amazon_link2=(bb.get("link")).encode("utf-8")
		            amazon_subscription_id=re.findall("\w+\d+\w+", amazon_link2)[0]
		            amazon_subscription_id_arr.append(amazon_subscription_id)
		            break
		        else:
		            amazon_subscription_flag='False' 
		else:
		...
		if ii.get("tv_everywhere_web_sources")!=[] and ii.get("tv_everywhere_web_sources") is not None:
		    ...
		    for bb in tv_everywhere:
		        if "amazon_buy" in bb.get('source') or "amazon_prime" in bb.get('source'):
		            amazon_tv_everywhere_flag='True'
		            amazon_link3=(bb.get("link")).encode("utf-8")
		            amazon_tv_everywhere_id=re.findall("\w+\d+\w+", amazon_link3)[0]
		            amazon_tv_everywhere_id_arr.append(amazon_tv_everywhere_id)
		            break
		        else:
		            amazon_tv_everywhere_flag='False'
		else:
		...
		if ii.get("free_web_sources")!=[] and ii.get("free_web_sources") is not None:
		    ...
		    for bb in free_web:
		        if "amazon_buy" in bb.get('source') or "amazon_prime" in bb.get('source'):
		            amazon_free_web_flag='True'
		            amazon_link4=(bb.get("link")).encode("utf-8")
		            amazon_free_web_id=re.findall("\w+\d+\w+", amazon_link4)[0]
		            amazon_free_web_id_arr.append(amazon_free_web_id)
		            break 
		        else:
		            amazon_free_web_flag='False'
		else:
		    free_web_flag='False'          
		    amazon_free_web_flag='False'
		if (purchase_flag=='False' or amazon_purchase_flag=='False') and (free_web_flag=='False' or amazon_free_web_flag=='False') and (tv_everywhere_flag=='False' or amazon_tv_everywhere_flag=='False') and (subscription_flag=='False' or amazon_subscription_flag=='False'):
		    y=y+1  
                    multiple_id_flag_amazon='Null' 
		    writer.writerow({"gb_id":gb_id,"show_type":show_type,"title":title,"description":description,"original_title":original_title,"series_title":series_title,"alternate_titles":alternate_titles,"season_number":season_number,"episode_number":episode_number,"release_year":release_year,"air_date":air_date,"duration":duration,"content_type":content_type,"updated_at_db":updated_at_db,"purchase_flag":purchase_flag,"subscription_flag":subscription_flag,"tv_everywhere_flag":tv_everywhere_flag,"free_web_flag":free_web_flag,"amazon_purchase_flag":amazon_purchase_flag,"amazon_purchase_id":amazon_purchase_id_arr,"amazon_subscription_flag":amazon_subscription_flag,"amazon_subscription_id":amazon_subscription_id_arr,"amazon_tv_everywhere_flag":amazon_tv_everywhere_flag,"amazon_tv_everywhere_id":amazon_tv_everywhere_id_arr,"amazon_free_web_flag":amazon_free_web_flag,"amazon_free_web_id":amazon_free_web_id_arr,"multiple_id_flag_amazon":multiple_id_flag_amazon,"Amazon_link_projectx_id":'',"comment":'No links in Database'})
		#import pdb;pdb.set_trace()
		try:
		    if amazon_purchase_flag=='True' or amazon_subscription_flag=='True' or amazon_tv_everywhere_flag=='True' or amazon_free_web_flag=='True':
		        amazon_arr=amazon_purchase_id_arr+amazon_subscription_id_arr+amazon_tv_everywhere_id_arr+amazon_free_web_id_arr
		        for jj in amazon_arr:
		            if amazon_arr.count(jj)>1:
		                amazon_arr.remove(jj)
		            if jj in amazon_arr:
		               if amazon_arr.count(jj)>1:
		                   amazon_arr.remove(jj)
		    
		        url_link="http://34.231.212.186:81/projectx/%s/amazon/ottprojectx" %amazon_arr[0]
		            ...
		            if data_resp_link:
		                for tt in data_resp_link:
		                    if tt.get("sub_type")=="MO" and tt.get("type")=='Program' and tt.get("data_source")=='GuideBox':
		                        arr_px_amazon.append(tt.get("projectx_id"))
		                        arr_gb_amazon.append(tt.get("source_id"))
		                    if tt.get("type")=='Program' and tt.get("data_source")=='Rovi':
		                        arr_px_amazon.append(tt.get("projectx_id"))
		                        arr_rovi_amazon.append(tt.get("source_id"))
		            else:
		                comment1='amazon link is not ingested'
		            for aa in arr_px_amazon:
		                if arr_px_amazon.count(aa)>1:
		                    arr_px_amazon.remove(aa)
		            for aa in arr_rovi_amazon:
		                if arr_rovi_amazon.count(aa)>1:
		                    arr_rovi_amazon.remove(aa)
		            for aa in arr_gb_amazon:
		                if arr_gb_amazon.count(aa)>1:
		                    arr_gb_amazon.remove(aa)
		            for jj in arr_px_amazon:
		                sec_arr_amazon.append(jj)
		            if len(sec_arr_amazon)>1:
                                px_link="http://preprod-projectx-1556298832.us-east-1.elb.amazonaws.com/programs?ids=%s&ott=true" %'{}'.format(",".join([str(i) for i in sec_arr_amazon]))
                                ...
                                for hh in data_resp_link1:
                                    if hh.get("variant_parent_id") is not None and hh.get("title_parent_id") is not None:
                                        if hh.get("variant_parent_id")==hh.get("id"):
                                            comment1='multiple projectx ids found in amazon movie'
                                            multiple_id_flag_amazon='True'
                                        else:
                                            try:
                                                sec_arr_amazon.remove(hh.get("variant_parent_id"))
                                            except ValueError:
		        ...
		        if show_type=='SE':
		            ...
		            if len(sec_arr_amazon)>1:
                                ...
                                for hh in data_resp_link1:
                                    if hh.get("variant_parent_id") is not None and hh.get("title_parent_id") is not None:
                                        if hh.get("variant_parent_id")==hh.get("id"):
                                            comment1='multiple projectx ids found in amazon episode'
                                            multiple_id_flag_amazon='True'
                                        else:
                                            try:
                                                sec_arr_amazon.remove(hh.get("variant_parent_id"))
		                ...
		                multiple_id_flag_amazon='False'
		    if multiple_id_flag_amazon=='True':
                        x=x+1
		        writer.writerow({"gb_id":gb_id,"show_type":show_type,"title":title,"description":description,"original_title":original_title,"series_title":series_title,"alternate_titles":alternate_titles,"season_number":season_number,"episode_number":episode_number,"release_year":release_year,"air_date":air_date,"duration":duration,"content_type":content_type,"updated_at_db":updated_at_db,"purchase_flag":purchase_flag,"subscription_flag":subscription_flag,"tv_everywhere_flag":tv_everywhere_flag,"free_web_flag":free_web_flag,"amazon_purchase_flag":amazon_purchase_flag,"amazon_purchase_id":amazon_purchase_id_arr,"amazon_subscription_flag":amazon_subscription_flag,"amazon_subscription_id":amazon_subscription_id_arr,"amazon_tv_everywhere_flag":amazon_tv_everywhere_flag,"amazon_tv_everywhere_id":amazon_tv_everywhere_id_arr,"amazon_free_web_flag":amazon_free_web_flag,"amazon_free_web_id":amazon_free_web_id_arr,"Rovi_id":{"amazon":arr_rovi_amazon},"multiple_id_flag_amazon":multiple_id_flag_amazon,"Amazon_link_projectx_id":sec_arr_amazon,"comment":comment1})

		    if multiple_id_flag_amazon=='False':
                        z=z+1
		        writer.writerow({"gb_id":gb_id,"show_type":show_type,"title":title,"description":description,"original_title":original_title,"series_title":series_title,"alternate_titles":alternate_titles,"season_number":season_number,"episode_number":episode_number,"release_year":release_year,"air_date":air_date,"duration":duration,"content_type":content_type,"updated_at_db":updated_at_db,"purchase_flag":purchase_flag,"subscription_flag":subscription_flag,"tv_everywhere_flag":tv_everywhere_flag,"free_web_flag":free_web_flag,"amazon_purchase_flag":amazon_purchase_flag,"amazon_purchase_id":amazon_purchase_id_arr,"amazon_subscription_flag":amazon_subscription_flag,"amazon_subscription_id":amazon_subscription_id_arr,"amazon_tv_everywhere_flag":amazon_tv_everywhere_flag,"amazon_tv_everywhere_id":amazon_tv_everywhere_id_arr,"amazon_free_web_flag":amazon_free_web_flag,"amazon_free_web_id":amazon_free_web_id_arr,"Rovi_id":{"amazon":arr_rovi_amazon},"multiple_id_flag_amazon":multiple_id_flag_amazon,"Amazon_link_projectx_id":sec_arr_amazon,"comment":comment1})


                    print ({"total":total, "amazon_link not ingested": y, "multiple ids": x, "single ids": z, "thread_name":name}) 
		    ...
		    continue    
    connection.close()
    print ({"total":total, "amazon_link not ingested": y, "multiple ids": x, "single ids": z})  
