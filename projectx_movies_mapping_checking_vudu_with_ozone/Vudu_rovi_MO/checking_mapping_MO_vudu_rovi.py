...
def main(start_id,thread_name,end_id,page_id):
    inputFile="/input/vudu_movie_mappings"
    ...
    with output_file as mycsvfile:
        ...
        for r in range(start_id,end_id):
            ...
            vudu_id=str(input_data[r][1])
            rovi_id=str(input_data[r][2])
            #import pdb;pdb.set_trace()
            try:
                px_ids=getting_px_ids(vudu_id,rovi_id,thread_name,initialization_file.source)
                checking_mapping_movies.checking_mapping_movies(vudu_id,rovi_id,px_ids["source_projectx_id"],
