...
def checking_mapping_episodes(source_id,rovi_id,source_sm_id,episode_title,
                             ozoneepisodetitle,OzoneOriginalEpisodeTitle,scheme,rovi_series_id,rovi_projectx_id,
                                     source_projectx_id,source_projectx_id_sm,writer,thread_name,source):
    #import pdb;pdb.set_trace()
    dev_rovi_px_mapped=[]
    ...
    #TODO: condition 5, to check source id ingested and checking for another source_ids    
        ...
        try:
            ...
            if data_resp_url==[]:
                id_api=initialization_file.api_check_presence_data_sm%(src,source_sm_id)
                data_resp_url_source=fetch_api_response(id_api)
                if data_resp_url_source==True:
                    ...
                    initialization_file.id_not_present_in_DB_count=initialization_file.id_not_present_in_DB_count+1
                    writer.writerow({"%s_sm_id"%source:source_sm_id,"%s_id"%source:source_id,"Rovi_id":rovi_id,"Rovi_SM_id":rovi_series_id,"episode_title":episode_title,
                                     "Scheme":scheme,"ozoneepisodetitle":ozoneepisodetitle,"OzoneOriginalEpisodeTitle":OzoneOriginalEpisodeTitle,"projectx_id_rovi":str(rovi_projectx_id[0]),
            ...
            if retry_count<=5:
                checking_mapping_episodes(source_id,rovi_id,source_sm_id,episode_title,ozoneepisodetitle,OzoneOriginalEpisodeTitle,scheme,rovi_series_id,writer,thread_name,source)
            else:
               retry_count=0    
    
    print("\n")
    print("thread thread_name:",thread_name,"multiple mapped: ",initialization_file.multiple_mapped_count,"mapped :",initialization_file.mapped_count ,
          "Not mapped :", initialization_file.not_mapped_count, "rovi id not ingested :", initialization_file.rovi_id_not_ingested_count, 
           "source Id not ingested : ",initialization_file.source_id_not_ingested_count, "both not ingested: ",initialization_file.both_source_not_ingested_count, 
             "id not present in db:" ,initialization_file.id_not_present_in_DB_count)
    print("\n")
