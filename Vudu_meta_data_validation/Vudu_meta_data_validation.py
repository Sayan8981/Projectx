...
class meta_data:
    ...
    def getting_source_details(self,vudu_id,show_type,source,thread_name,details):
        #import pdb;pdb.set_trace()
        retry_count=0
        ...
        try:
            ...
            vudu_images_details=self.getting_images(vudu_id,show_type,vudu_show_id,details)
            ...
            print ("\n")    
            if retry_count<=5:
                self.getting_source_details(vudu_id,show_type,source,thread_name,details)    
            else:
                retry_count=0
...
class mata_data_validation_vudu:
    ...
    def meta_data_validation_(self,data,projectx_id,source_id,show_type,thread_name,only_mapped_ids):
        #import pdb;pdb.set_trace()
        source_details=meta_data().getting_source_details(source_id,show_type,self.source,thread_name,data)
        ...
        meta_data_validation_result=ott_meta_data_validation_modules().meta_data_validate_vudu()
                        .meta_data_validation(source_id,source_details,projectx_details,show_type)
        ...
        try:
            writer.writerow([source_id,projectx_id,show_type,source_details["source_title"],projectx_details["px_long_title"],projectx_details["px_episode_title"],meta_data_validation_result["title_match"],meta_data_validation_result["description_match"],meta_data_validation_result["genres_match"]
    ...
    def main(start_id,thread_name,end_id,sourcetable,source):
        import pdb;pdb.set_trace()
        self.get_env_url()
    ...
    def threading_pool(self):    
        ...
        t1=threading.Thread(target=self.main,args=(1,"thread-1",20000))
        t1.start()
        """t2=threading.Thread(target=self.main,args=(20000,"thread-2",40000))
        t2.start()
        t3=threading.Thread(target=self.main,args=(40000,"thread-3",60000))
        t3.start()
        t4=threading.Thread(target=self.main,args=(60000,"thread-4",80000))
        t4.start()
        t5=threading.Thread(target=self.main,args=(80000,"thread-5",100000))
        t5.start()
        t6=threading.Thread(target=self.main,args=(100000,"thread-6",120000))
        t6.start()
        t7=threading.Thread(target=self.main,args=(120000,"thread-7",140000))
        t7.start()
        t8=threading.Thread(target=self.main,args=(140000,"thread-8",160000))
        t8.start()
        t9=threading.Thread(target=self.main,args=(160000,"thread-9",180000))
        t9.start()
        t10=threading.Thread(target=self.main,args=(180000,"thread-10",200000))
        t10.start()
        t11=threading.Thread(target=self.main,args=(200000,"thread-11",240000))
        t11.start()"""
