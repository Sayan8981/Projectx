...
def main(start_id,thread_name,end_id,page_id):
    inputFile="/input/vudu_episode_mappings"
    #import pdb;pdb.set_trace()
    ...
    with output_file as mycsvfile:
        ...
        for data in range(start_id,end_id):
            ...
            vudu_id=eval(input_data[data][2])
            rovi_id=eval(input_data[data][4])
            vudu_sm_id=eval(input_data[data][1])
            print("\n")
            print ("Total:", initialization_file.total, "thread_name:", thread_name,"start:",start_id,"end:",end_id,"vudu_sm_id:",vudu_sm_id,"vudu_id:",vudu_id,"rovi_id:",rovi_id)
            rovi_series_id=eval(input_data[data][3])
            try:
                px_ids=getting_px_ids(vudu_id,rovi_id,thread_name,initialization_file.source)
                checking_mapping_episodes.checking_mapping_episodes(vudu_id,rovi_id,vudu_sm_id,rovi_series_id,px_ids["rovi_projectx_id"],px_ids["source_projectx_id"],
                                           source_projectx_id_sm,writer,thread_name,initialization_file.source)
            except (Exception,httplib.BadStatusLine,urllib2.HTTPError,socket.error,urllib2.URLError) as e:
                retry_count+=1
                print ("Exception caught ......................",type(e),vudu_id,rovi_id)
                if retry_count<=5:
                    px_ids= getting_px_ids(vudu_id,rovi_id,thread_name,initialization_file.source)
                    checking_mapping_episodes.checking_mapping_episodes(vudu_id,rovi_id,vudu_sm_id,rovi_series_id,px_ids["rovi_projectx_id"],px_ids["source_projectx_id"],
                                               source_projectx_id_sm,writer,thread_name,initialization_file.source)
