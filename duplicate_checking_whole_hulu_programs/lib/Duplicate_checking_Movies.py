...
class ProjectXMovies:
    ...
    def reverse_api_extract(self, link_details,source):
        ...
        for kk in reverse_api_resp:
            ...
            if kk.get("data_source")==source and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))
            if (kk.get("data_source")=='GuideBox' or kk.get("data_source")=='Vudu') and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))        
        ...
        if duplicate=='False' or duplicate=='':
            ...
            if search_px_id1:
                if len(search_px_id__tmp)>1 and duplicate=='True':
                    ...
                    self.comment_variant_parent_id=variant_result[0]
                    self.comment_variant_parent_id_present=variant_result[1]
                    writer.writerow([source,'', self.id,self.launch_id,'MO',self.movie_title,'','',self.release_year,self.link_expired,self.px_id,'','','',self.comment,self.comment,'',search_px_id__tmp,'','','','','','','',self.comment_variant_parent_id_present,self.comment_variant_parent_id,'',''])            
            else:
                ...
                writer.writerow([source, '', self.id,self.launch_id,'MO', self.movie_title, '', '', 
                                 self.release_year, self.link_expired, self.px_id, '', '', '', 
                                 self.comment, self.comment, duplicate, search_px_id__tmp])   
    ...
    def duplicate_checking_movies(self, movie_data, source, writer,writer1): 
        ...
        self.id=movie_data.get("site_id")
        self.launch_id=movie_data.get("id")
        self.release_year=movie_data.get("original_premiere_date").split("-")[0]
        ...
        print ("projectx_ids : {}, hulu_mo_id: {}".format(self.px_id,self.id))
        if len(self.px_id) > 1:               
