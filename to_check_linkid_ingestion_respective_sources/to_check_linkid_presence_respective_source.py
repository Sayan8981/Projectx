...
class linkid_checking:
    ...
    def cleanup(self):
        ...
        self.prod_videos_response=[] 
        self.preprod_videos_response=[]
        self.same_rovi_id_present='None'  
        self.prod_launch_id=[] 
        self.prod_video_link=[]
        self.px_id_present='True'
        self.prod_link_status='None'
        self.preprod_launch_id=[] 
        self.preprod_video_link=[]
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        self.column_fieldnames=["Service_name","Link_id","Rovi_id","Projectx_id","Rovi_id_form_reverse_api","Rovi_id_match","Link_expired",
           "Prod_link_status","Preprod_link_status","Link_fetched_from","Fetched_from_sources","Px_id_present","Reverse_api_response_stats",
           "Comment"]
        with output_file as mycsvfile:
            ...
            for id_ in range(start_id,end_id):
                ...
                self.writer.writerow([self.service,self.link_id,self.rovi_id,self.px_id,self.source_id,self.same_rovi_id_present,
                    self.link_expired,self.prod_link_status,self.preprod_link_status,self.fetched_source,self.fetch_from,self.px_id_present,
                    self.reverse_api_response_status,self.comment])
        output_file.close() 

    # TODO: multi process Operations 
    def thread_pool(self): 
        t1=Process(target=self.main,args=(1,"process - 1",5))
        t1.start()
