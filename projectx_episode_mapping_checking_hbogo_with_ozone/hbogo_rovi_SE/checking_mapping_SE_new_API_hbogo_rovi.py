...
def main(start_id,thread_name,end_id,page_id):
    inputFile="/input/hbogo_episode_mappings"
    #import pdb;pdb.set_trace()
    ...
    with output_file as mycsvfile:
        ...
        for data in range(start_id,end_id):
            ...
            hbogo_id=eval(input_data[data][2])
            rovi_id=eval(input_data[data][4])
            hbogo_sm_id=eval(input_data[data][1])
            print("\n")
            print ("Total:", initialization_file.total, "thread_name:", thread_name,"start:",start_id,"end:",end_id,"hbogo_sm_id:",hbogo_sm_id,"hbogo_id:",hbogo_id,"rovi_id:",rovi_id)
            rovi_series_id=eval(input_data[data][3])
            try:
                px_ids=getting_px_ids(hbogo_id,rovi_id,thread_name,initialization_file.source)
                checking_mapping_episodes.checking_mapping_episodes(hbogo_id,rovi_id,hbogo_sm_id,rovi_series_id,px_ids["rovi_projectx_id"],px_ids["source_projectx_id"],
