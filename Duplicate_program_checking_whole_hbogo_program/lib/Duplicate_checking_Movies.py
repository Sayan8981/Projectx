...
class ProjectXMovies:
    ...
    def expiry_check_response(self,expired_api):
        try:
            ...
            ## TODO: Validate response before returning
            return data_resp
        except (Exception,HTTPError,URLError,httplib.BadStatusLine,socket.error) as e:
            print ("\n Retrying...................")
    ...
    def reverse_api_extract(self, link_details,source):
        ...
        for kk in reverse_api_resp:
            ...
            if kk.get("data_source")==source and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))
            if (kk.get("data_source")=='GuideBox' or kk.get("data_source")=='Hulu' or kk.get("data_source")=='Vudu') and kk.get("type")=='Program' and kk.get("sub_type")=='MO':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))        
        ...
        if duplicate=='False' or duplicate=='':
            ...
            if search_px_id1:
                if len(search_px_id__tmp)>1 and duplicate=='True':
                    ...
                    #import pdb;pdb.set_trace()
                    projectx_api=initialization_file.projectx_preprod_api%'{}'.format(",".join([str(i) for i in self.px_id]))
                    ...
                    self.comment_variant_parent_id=variant_result[0]
                    self.comment_variant_parent_id_present=variant_result[1]
                    writer.writerow([source,'', self.id,'MO',self.movie_title,'','',self.release_year,self.link_expired,self.px_id,'','','',self.comment,self.comment,'',search_px_id__tmp,'','','','','','','',self.comment_variant_parent_id_present,self.comment_variant_parent_id,'',''])            
            else:
                duplicate_api=initialization_file.duplicate_api%(self.id,source,'MO')
    ...
    def search_api_call_response(self, px_id, source, link_expired, writer,writer1,object_):
        #import pdb;pdb.set_trace() 
        next_page_url=""
        duplicate=""
        data_resp_search=dict()

        search_api=initialization_file.projectx_preprod_search_api%urllib2.quote(self.movie_title)
            ...
            for nn in data_resp_search.get("results"):
                if nn.get("action_type")=="ott_search" and (nn.get("results")==[] or nn.get("results")):
                    ...
                    if next_page_url is not None: 
                        ...
                        data_resp_search = self.fetch_from_cloud_for_id(search_api)
                        self.search_api_response_validation(data_resp_search, source, px_id, link_expired, writer, duplicate,writer1,object_)
                    else:    
    ...
    def duplicate_checking_movies(self, movie_data,id_, source, writer,writer1,object_): 
        #import pdb;pdb.set_trace()     
        print("\nFunction duplicate_checking_movies_called")
        self.id=(movie_data[id_])[0]
        self.movie_title=unidecode.unidecode(pinyin.get((movie_data[id_])[2]))
        self.release_year=(movie_data[id_])[9]
        ...
        print ("projectx_ids : {}, hbogo_mo_id: {}".format(self.px_id,self.id))
        if len(self.px_id) > 1:               
            self.search_api_call_response(self.px_id,source,self.link_expired,writer,writer1,object_)
        else:
