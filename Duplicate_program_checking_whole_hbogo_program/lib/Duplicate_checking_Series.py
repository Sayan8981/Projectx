...
class ProjectXSeries:
    ...
    def fetch_from_cloud_for_id(self, api):
        ...
        try:
            ...
            return data_resp
        except (Exception,URLError,HTTPError,httplib.BadStatusLine,socket.error) as e:
            print ("\n Retrying...................")
    ...
    def expiry_check(self, link_details):
        ...
        expired_api=initialization_file.preprod_version_expire_api%(link_details,'HBOGO')
    ...
    def reverse_api_extract(self, link_details,source):
        ...
        for kk in reverse_api_resp:
            ...
            if kk.get("data_source")==source and kk.get("type")=='Program' and kk.get("sub_type")=='SE':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))
            if (kk.get("data_source")=='GuideBox' or kk.get("data_source")=='Hulu' or kk.get("data_source")=='Vudu') and kk.get("type")=='Program' and kk.get("sub_type")=='SE':
                if kk.get("projectxId") not in self.px_id:      
                    self.px_id.append(kk.get("projectxId"))        

    def duplicate_results(self,search_px_id__tmp,source,writer,writer1,object_):
        rovi_id=[]
        rovi_mapping=[]
        source_mapping=[]
        guidebox_mapping=[]
        if (self.credit_match=='False' or self.credit_match=='') and len(search_px_id__tmp)==2:
            #import pdb;pdb.set_trace()
            px_link=initialization_file.projectx_preprod_api %'{}'.format(",".join([str(i) for i in search_px_id__tmp]))
        ...
        for aa in search_px_id__tmp:
           projectx_mapping_api=initialization_file.projectx_mapping_api%aa
           ...
           for yy in data_resp_mapped:
               ...
               if yy.get("data_source")==source and yy.get("type")=='Program' and yy.get('sub_type')=='SM':
                   ...
                   source_mapping.append({str(aa):"%s_id:"%source+yy.get("sourceId")})
        ...
        if self.count_rovi>1:
            ...
            preprod_api=initialization_file.preprod_beta_api%'{}'.format(",".join([str(i) for i in rovi_id]))
        ...
        if duplicate=='False' or duplicate=='':
            ...
            if search_px_id1:
                if len(search_px_id__tmp)>1 and duplicate=='True':
                    self.duplicate_results(search_px_id__tmp,source,writer,writer1,object_)        
                else:
                    self.comment='Duplicate ids not found in search api'
                    #import pdb;pdb.set_trace()
                    projectx_api=initialization_file.projectx_preprod_api%'{}'.format(",".join([str(i) for i in self.series_id_px]))
                    ...
                    writer.writerow([source,self.show_id,self.id,'SE','',self.series_title,self.episode_title,
                                    self.release_year,self.link_expired,self.series_id_px,'','','',self.comment,self.comment
                                    ,'',search_px_id__tmp,'','','','','','','',self.comment_variant_parent_id_present,
                                      self.comment_variant_parent_id,'',''])            
            else:
                duplicate_api=initialization_file.duplicate_api%(self.show_id,source,'SM')
    ...
    def search_api_call_response(self, series_id_px, source, link_expired, writer,writer1,object_):
        #import pdb;pdb.set_trace() 
        next_page_url=""
        duplicate=""
        data_resp_search=dict()

        search_api=initialization_file.projectx_preprod_search_api%urllib2.quote(self.series_title)
            ...
            for nn in data_resp_search.get("results"):
                if nn.get("action_type")=="ott_search" and (nn.get("results")==[] or nn.get("results")):
                    ...
                    if next_page_url is not None: 
                        ...
                        data_resp_search = self.fetch_from_cloud_for_id(search_api)
                        self.search_api_response_validation(data_resp_search, source, series_id_px, link_expired, writer, duplicate,writer1,object_)
                    else:    
                        data_resp_search={"results":[]}
                else:
                    data_resp_search={"results":[]}                          

    def duplicate_checking_series(self, series_data,id_, source, writer,writer1,object_): 
        #import pdb;pdb.set_trace()     
        print("\nFunction duplicate_checking_series_called")
        self.id=eval((series_data[id_])[0])
        self.show_id=eval((series_data[id_])[4])
        self.release_year=(series_data[id_])[9]
        self.series_title=unidecode.unidecode(pinyin.get((series_data[id_])[3]))
        self.episode_title=unidecode.unidecode(pinyin.get((series_data[id_])[2]))
        ...
        if self.series_title is not None:
            ...
            self.reverse_api_extract(self.id, source)
            print ("projectx_ids : {},hbogo_se_id: {}, hbogo_SM_id: {}".format(self.px_id,self.id,self.show_id))
            if len(self.px_id) > 1:
                px_link=initialization_file.projectx_preprod_api%'{}'.format(",".join([str(i) for i in self.px_id]))
                ...
                if len(self.series_id_px)>1:               
                    self.search_api_call_response(self.series_id_px,source,self.link_expired,writer,writer1,object_)
                else:
