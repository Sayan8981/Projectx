...
#TODO: main func 
def main():
    inputFile="/input/vudu_series_mapping"   
    ...
    with output_file as mycsvfile:
        ...
        for data in range(1,len(input_data)):
            ...
            vudu_sm_id=eval(input_data[data][0])
            rovi_sm_id=eval(input_data[data][1])
            print ({"total":initialization_file.total,"rovi_sm_id":rovi_sm_id,"vudu_sm_id":vudu_sm_id})
            try:
                px_ids=getting_px_ids(vudu_sm_id,rovi_sm_id,initialization_file.source)
                checking_mapping_series.checking_mapping_series(vudu_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                 px_ids["rovi_projectx_id"],writer,initialization_file.source)
            except (Exception,httplib.BadStatusLine,urllib2.HTTPError,socket.error,urllib2.URLError) as e:
                #import pdb;pdb.set_trace()
                retry_count+=1
                print ("Exception caught ......................",type(e),vudu_sm_id,rovi_sm_id)
                if retry_count<=5:
                   px_ids= getting_px_ids(vudu_sm_id,rovi_sm_id,initialization_file.source)
                   checking_mapping_series.checking_mapping_series(vudu_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                    px_ids["rovi_projectx_id"],writer,initialization_file.source)
