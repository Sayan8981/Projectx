...
class ProjectXSeries:
        ...
        if duplicate=='False' or duplicate=='':
            ...
            if search_px_id1:
                if len(search_px_id__tmp)>1 and duplicate=='True':
                    ...
                    self.comment_variant_parent_id=variant_result[0]
                    self.comment_variant_parent_id_present=variant_result[1]
                    writer.writerow([source,self.id,self.show_id,'SE',self.link_details,'',self.series_title,self.episode_title,self.release_year,self.link_expired,self.px_id,'','','',self.comment,self.comment,'',search_px_id__tmp,'','','','','','','',self.comment_variant_parent_id_present,self.comment_variant_parent_id,'',''])            
            else:
                duplicate_api=initialization_file.duplicate_domain%(self.show_id,source,'SM')
                ...
                writer.writerow([source, self.id,self.show_id,'SE', self.link_details,'', self.series_title, self.episode_title, 
    ...
    def duplicate_checking_series(self, series_data, source, writer,writer1): 
        ...
        self.id=series_data.get("gb_id")
        self.release_year = series_data.get("release_year")
        self.show_id=series_data.get("show_id")
        self.episode_title=unidecode.unidecode(pinyin.get(series_data.get("title")))
        if series_data.get("series_title") is not None:
            self.series_title = unidecode.unidecode(pinyin.get(series_data.get("series_title")))
            self.extract_link_details(self.fetch_from_cloud_for_id(initialization_file.gb_details_api%(self.id, 'SE')))
  
            if self.link_details and self.link_details != 'None':
                ...
                print ("projectx_ids_episodes : {0}".format(self.px_id))
                if len(self.px_id) > 1: 
                    ...
                    if len(self.series_id_px)>1:                      
                        ...
                        self.comment=('No multiple ids for series',self.link_details)
                        self.result="No multiple ids for this series"
                        writer.writerow([source, self.id,self.show_id,'SE', self.link_details, '',self.series_title, 
                                     self.episode_title, self.release_year, self.link_expired, self.series_id_px, 
                                     '', '', '' ,self.comment,self.result, '', '', '', '', ''])    
                else:
                    self.comment=('No multiple ids for this episode',self.link_details)
                    self.result="No multiple ids for this episode"
                    writer.writerow([source, self.id,self.show_id,'SE', self.link_details, '',self.series_title, 
                                 self.episode_title, self.release_year, self.link_expired, self.px_id, 
                                 '', '', '' ,self.comment,self.result, '', '', '', '', '']) 
            else:
                self.comment=('No links for this episode')
                self.result="No links for this episode"
                writer.writerow([source, self.id,self.show_id,'SE', '', '',self.series_title, 
                             self.episode_title, self.release_year, '', '', 
                             '', '', '' ,self.comment,self.result, '', '', '', '', ''])        
        else:
            self.comment=('No series_title for this episode')
            self.result="No series title for this episode"
            writer.writerow([source, self.id,self.show_id,'SE', '', '',self.series_title, 
                         self.episode_title, self.release_year, '', '', 
                         '', '', '' ,self.comment,self.result, '', '', '', '', ''])          
