...
def main(start_id,thread_name,end_id,page_id):
    inputFile="/input/showtime_movie_mappings"
    ...
    with output_file as mycsvfile:
        ...
        for r in range(start_id,end_id):
            #import pdb;pdb.set_trace()
            print("\n")
            print ("here checking the rovi and showtime movies program mapping...................... ",thread_name)
            initialization_file.total=initialization_file.total+1
            showtime_id=str(input_data[r][1])
            rovi_id=str(input_data[r][2])
            #import pdb;pdb.set_trace()
            try:
                px_ids=getting_px_ids(showtime_id,rovi_id,thread_name,initialization_file.source)
                checking_mapping_movies.checking_mapping_movies(showtime_id,rovi_id,px_ids["source_projectx_id"],
