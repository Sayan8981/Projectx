...
def getting_px_ids(source_sm_id,rovi_sm_id,source):
    ...
    try:
        ...
        #TODO: going for mapping validation
...
def main():
    inputFile="/input/HuluSeriesMappping"
    result_sheet='/output/Result_mapping_SM.csv'
    ...
    with output_file as mycsvfile:        
        writer = csv.DictWriter(mycsvfile,fieldnames=fieldnames_tag,dialect="csv",lineterminator = '\n')
        writer.writeheader()
        for data in range(1,len(input_data)):
            initialization_file.total=initialization_file.total+1
            hulu_sm_id=eval(input_data[data][0])
            rovi_sm_id=eval(input_data[data][1])
            print ("hulu_sm_id:",hulu_sm_id,"rovi_sm_id:",rovi_sm_id,"total:",initialization_file.total)
            try:
                px_ids=getting_px_ids(hulu_sm_id,rovi_sm_id,initialization_file.source)
                checking_mapping_series.checking_mapping_series(hulu_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                    px_ids["rovi_projectx_id"],writer,initialization_file.source)
            except (Exception,httplib.BadStatusLine,urllib2.HTTPError,socket.error,urllib2.URLError) as e:
                #import pdb;pdb.set_trace()
                retry_count+=1
                print ("Exception caught ......................",type(e),hulu_sm_id,rovi_sm_id)
                if retry_count<=5:
                    px_ids=getting_px_ids(hulu_sm_id,rovi_sm_id,initialization_file.source)
                    checking_mapping_series.checking_mapping_series(hulu_sm_id,rovi_sm_id,px_ids["source_projectx_id"],
                                        px_ids["rovi_projectx_id"],writer,initialization_file.source)
