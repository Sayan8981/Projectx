...
class movies_mapp_fail_cases:
    ...
    def __init__(self):
        self.fieldnames= ["rovi_id","vudu_id","px_id1","px_id1_show_type","px_id1_variant_parent_id","px_id1_is_group_language_primary",
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        self.default_param()
        inputFile="input_movies_mappFail/vudu_movies_mapp_fail_input_preprod"
        input_data=lib_common_modules().read_csv(inputFile)

        result_sheet='/output_automated_fail_cases_movies/result_preprod_%s_%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as outputcsvfile:
            ...
            for data in range(start_id,end_id):
                print("Checking same program of map fail movies.............")
                self.cleanup()
                self.rovi_id=eval(input_data[data][0])
                self.vudu_id=eval(input_data[data][1])
                self.px_id1=eval(input_data[data][2])
                self.px_id2=eval(input_data[data][3])
                #import pdb;pdb.set_trace()
                if type(self.px_id1)!=list and type(self.px_id2)!=list:
                    self.px_array.insert(0,self.px_id1)
                    self.px_array.insert(1,self.px_id2)
                    print("\n")
                    self.total +=1
                    print(self.rovi_id,self.vudu_id,[self.px_id1,self.px_id2],thread_name,"total:",self.total)
                    ...
                    try:
                        result.update({"rovi_id":self.rovi_id,"vudu_id":self.vudu_id})
    ...
    def thread_pool(self):
        t1=Process(target=self.main,args=(1,"thread-1",500))
        t1.start()
        t2=Process(target=self.main,args=(500,"thread-2",1110))
        t2.start()
        # t3=Process(target=self.main,args=(1000,"thread-3",1500))
        # t3.start()
        # t4=Process (target=self.main,args=(1500,"thread-4",2880))
        # t4.start()
