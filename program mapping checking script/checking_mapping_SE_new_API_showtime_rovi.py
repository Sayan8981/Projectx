...
def open_csv():
    inputFile="showtime_rovi_mapping_episodes"
    f = open(os.getcwd()+'/'+inputFile+'.csv', 'rb')
    reader = csv.reader(f)
    fullist=list(reader)

    result_sheet='/Result_mapping_Series_API_showtime_rovi.csv'
    ...
    with w as mycsvfile:
	    ...
	    try:
	        ...
	        url_showtime="http://34.231.212.186:81/projectx/mappingfromsource?sourceIds=%d&sourceName=Showtime&showType=SE" %showtime_id
	    ...
	    if len(rovi_projectx_id)>1 or len(showtime_projectx_id)>1:
            ...
            if len(showtime_projectx_id)==0 and len(rovi_projectx_id)==1:
                z=z+1
                writer.writerow({"showtime_id":showtime_id,"Rovi_id":rovi_id,"projectx_id_rovi":str(rovi_projectx_id[0]),"projectx_id_showtime":'',"Episode mapping":'Fail',"Comment":'No Ingestion of gb_id of episode'})
