...
def open_csv(start,name,end,id):
                    ...
                    if data_resp_url==[]:
                        ...
                        if gb_projectx_id!=[]:
                            if rovi_projectx_id[0] in gb_projectx_id:
                                ...
                                k=k+1
                                writer.writerow({"Gb_sm_id":gb_sm_id,"Gb_id":gb_id,"Rovi_id":rovi_id,"episode_title":episode_title,"ozoneepisodetitle":ozoneepisodetitle,"OzoneOriginalEpisodeTitle":OzoneOriginalEpisodeTitle,"projectx_id_rovi":str(rovi_projectx_id),"projectx_id_gb":['not found match id'],"Episode mapping":'Fail',"Comment":'Fail'})       
    ...
    print("total id for both:", total,"multiple mapped: ",l,"mapped :",j ,"Not mapped :", k, "rovi id not ingested :", m+q, "gb Id not ingested : ",n+z, "both not ingested: ",p, "id not present in db:" ,gg)
...
t5.start()
t6 =threading.Thread(target=open_csv,args=(150001,"thread - 6",175001,6))
t6.start()
t7 =threading.Thread(target=open_csv,args=(175001,"thread - 7",205001,7))
t7.start()
t8 =threading.Thread(target=open_csv,args=(205001,"thread - 8",235001,8))
t8.start()
t9 =threading.Thread(target=open_csv,args=(235001,"thread - 9",265001,9))
t9.start()
t10 =threading.Thread(target=open_csv,args=(265001,"thread - 10",295001,10))
t10.start()
t11 =threading.Thread(target=open_csv,args=(295001,"thread - 11",325001,11))
t11.start()
t12 =threading.Thread(target=open_csv,args=(325001,"thread - 12",396787,12))
t12.start()
